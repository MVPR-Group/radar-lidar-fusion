...
class Accuracy(nn.Module):
    ...
    def forward(self, labels, preds, weights=None):
        ...
        if self._encode_background_as_zeros:
            ...
            labels_pred = torch.max(preds, dim=self._dim)[1] + 1      #torch.Size([3, 50000])
            pred_labels = torch.where((scores > self._threshold).any(self._dim), #torch.Size([3, 50000])
                                      labels_pred,
...
def _calc_binary_metrics(labels,
    ...
    N, *Ds = labels.shape
    labels = labels.view(N, int(np.prod(Ds)))     #torch.Size([3, 50000])
    pred_labels = pred_labels.view(N, int(np.prod(Ds)))  #torch.Size([3, 50000])
    pred_trues = pred_labels > 0
    pred_falses = pred_labels == 0
    trues = labels > 0
    falses = labels == 0
...
class PrecisionRecall(nn.Module):
    ...
    def forward(self, labels, preds, weights=None):
        ...
        if weights is None:
            ...
            weights = weights.float()
        for i, thresh in enumerate(self._thresholds):#<class 'list'>: [0.1, 0.3, 0.5, 0.7, 0.8, 0.9, 0.95]IOU
