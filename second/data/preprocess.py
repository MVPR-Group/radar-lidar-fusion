...
def merge_second_batch(batch_list):                     #将batch_size三个聚合成一个？
    ...
    for key, elems in example_merged.items():
        if key in [
                'voxels', 'radar_voxels','radar_num_points','num_points', 'num_gt', 'voxel_labels', 'gt_names', 'gt_classes', 'gt_boxes'
        ]:
        ...
        elif key == 'radar_coordinates':
            radar_coors = []
            for i, radar_coor in enumerate(elems):
                radar_coor_pad = np.pad(
                    radar_coor, ((0, 0), (1, 0)), mode='constant', constant_values=i)
                radar_coors.append(radar_coor_pad)
            ret[key] = np.concatenate(radar_coors, axis=0)
        elif key == 'metrics':
            ret[key] = elems
        else:
...
def prep_pointcloud(input_dict,
    ...
    radar_points = input_dict["radar"]["radar_points"]           #########
    if training:
    ...
    if remove_outside_points:                     #false
    ...
    if training:
        ...
        if db_sampler is not None:
            ...
            if sampled_dict is not None:
                ...
                if remove_points_after_sample:          #true
        ...
        prep.noise_per_object_v3_(             #加入噪声
            gt_dict["gt_boxes"],
            points,
            gt_boxes_mask,
        ...
        #                                                points, 0.5, random_flip_x, random_flip_y) #随机的对x,或y方向进行反转
        gt_dict["gt_boxes"], points, radar_points = prep.random_flip(gt_dict["gt_boxes"],
                                                       points, radar_points,0.5, random_flip_x, random_flip_y)  # 随机的对x,或y方向进行反转
        gt_dict["gt_boxes"], points = prep.global_rotation_v2(
            gt_dict["gt_boxes"], points, *global_rotation_noise)   #旋转噪声
        gt_dict["gt_boxes"], points = prep.global_scaling_v2(
            gt_dict["gt_boxes"], points, *global_scaling_noise)     #缩放噪声
        prep.global_translate_(gt_dict["gt_boxes"], points, global_translate_noise_std)     #加点噪声
        ...
        # cv2.waitKey(0)
    if shuffle_points:         # false
        # shuffle is a little slow.
        np.random.shuffle(points)

    # [0, -40, -3, 70.4, 40, 1]
    voxel_size = voxel_generator.voxel_size  # [0.25,0.25,20]上下10
    pc_range = voxel_generator.point_cloud_range
    grid_size = voxel_generator.grid_size
    # [352, 400]
    t1 = time.time()
    if not multi_gpu:
        res = voxel_generator.generate(            #380000
            points, max_voxels)
        radar_res = voxel_generator.generate(      #2100
            radar_points, max_voxels)
        voxels = res["voxels"]
        radar_voxels=radar_res["voxels"]                  #这个地方出了问题
        coordinates = res["coordinates"]
        radar_coordinates =radar_res["coordinates"]
        num_points = res["num_points_per_voxel"]          #每个柱子的点云点个数60
        radar_num_points = radar_res["num_points_per_voxel"]
        num_voxels = np.array([voxels.shape[0]], dtype=np.int64)  #6540
        num_radar_voxels = np.array([radar_voxels.shape[0]],dtype=np.int64)  #446
    else:
    ...
    example = {
        ...
        'num_radar_voxels':num_radar_voxels,
        "metrics": metrics,
    }
    ...
    if anchor_cache is not None:         #anchor_cache=None
    ...
    anchors_mask = None
    if anchor_area_threshold >= 0:          #anchor_area_threshold=1
