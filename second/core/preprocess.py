...
def random_flip(gt_boxes, points,radar_points, probability=0.5, random_flip_x=True, random_flip_y=True):
    ...
    flip_y = np.random.choice([False, True],
                              replace=False,
                              p=[1 - probability, probability])
    if flip_y and random_flip_y:      #随机的进行反转
...
def global_scaling_v2(gt_boxes, points, min_scale=0.95, max_scale=1.05):
    noise_scale = np.random.uniform(min_scale, max_scale)  #功能：从一个均匀分布[low,high)中随机采样，注意定义域是左闭右开，即包含low，不包含high
