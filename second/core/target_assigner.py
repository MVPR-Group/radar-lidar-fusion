...
class TargetAssigner:
    ...
    def generate_anchors(self, feature_map_size):
        ...
        idx = 0
        for anchor_generator, match_thresh, unmatch_thresh, fsize in zip(   #为10个类生成anchor
            ...
            anchors = anchors.transpose(ndim, *range(0, ndim), ndim + 1) #50个
            anchors_list.append(anchors.reshape(-1, self.box_ndim))
            num_anchors = np.prod(anchors.shape[:-1])  #5000个
            match_list.append(
                np.full([num_anchors], match_thresh, anchors.dtype))
            unmatch_list.append(
                np.full([num_anchors], unmatch_thresh, anchors.dtype))
            idx += 1
        anchors = np.concatenate(anchors_list, axis=0)         #50000个anchor,10个类的，一个5000
        matched_thresholds = np.concatenate(match_list, axis=0)   #10个类的阈值
