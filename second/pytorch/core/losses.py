  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights=None):
    ...
    Returns:
      loss: a float tensor of shape [batch_size, num_anchors] tensor
        representing the value of the loss function.
    """
    diff = prediction_tensor - target_tensor      #torch.Size([3, 50000, 7])
    if self._code_weights is not None:
      code_weights = self._code_weights.type_as(prediction_tensor).to(target_tensor.device)
      diff = code_weights.view(1, 1, -1) * diff
    abs_diff = torch.abs(diff)
    abs_diff_lt_1 = torch.le(abs_diff, 1 / (self._sigma**2)).type_as(abs_diff)     #逐元素比较 前者小于等于后者为1
    loss = abs_diff_lt_1 * 0.5 * torch.pow(abs_diff * self._sigma, 2) \
      + (abs_diff - 0.5 / (self._sigma**2)) * (1. - abs_diff_lt_1)           #torch.Size([3, 50000, 7])
    if self._codewise:
      anchorwise_smooth_l1norm = loss
      if weights is not None:
        anchorwise_smooth_l1norm *= weights.unsqueeze(-1)
    else:
      anchorwise_smooth_l1norm = torch.sum(loss, 2)   #  * weights         torch.Size([3, 50000, 7])
      if weights is not None:
        anchorwise_smooth_l1norm *= weights  #anchorwise_smooth_l1norm=torch.Size([3, 50000, 7])
    return anchorwise_smooth_l1norm

def _sigmoid_cross_entropy_with_logits(logits, labels):
  # to be compatible with tensorflow, we don't use ignore_idx
  loss = torch.clamp(logits, min=0) - logits * labels.type_as(logits)
  loss += torch.log1p(torch.exp(-torch.abs(logits)))      #log
...
class SigmoidFocalClassificationLoss(Loss):
  ...
  def _compute_loss(self,
    ...
    if class_indices is not None:
      weights *= indices_to_dense_vector(class_indices,           #torch.Size([3, 50000, 1])
            prediction_tensor.shape[2]).view(1, 1, -1).type_as(prediction_tensor)
    per_entry_cross_ent = (_sigmoid_cross_entropy_with_logits(     #torch.Size([3, 50000, 10])
        labels=target_tensor, logits=prediction_tensor))                   #log      prediction_tensor=torch.Size([3, 50000, 10])
    prediction_probabilities = torch.sigmoid(prediction_tensor)
    p_t = ((target_tensor * prediction_probabilities) +                     #torch.Size([3, 50000, 10])
           ((1 - target_tensor) * (1 - prediction_probabilities)))
    modulating_factor = 1.0
    if self._gamma:
      modulating_factor = torch.pow(1.0 - p_t, self._gamma)          #(1-p)**2  gamma=2
    alpha_weight_factor = 1.0
    if self._alpha is not None:                                          #alpha=0.25
      alpha_weight_factor = (target_tensor * self._alpha +
                              (1 - target_tensor) * (1 - self._alpha))

    focal_cross_entropy_loss = (modulating_factor * alpha_weight_factor *      #torch.Size([3, 50000, 10])
                                per_entry_cross_ent)
...
class WeightedSoftmaxClassificationLoss(Loss):
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    prediction_tensor = torch.div(             #torch.Size([3, 50000, 2])
