...
class VoxelNet(nn.Module):
    def __init__(self,
                 ...
                 # vfe_radar_class_name="VoxelradarFeatureExtractor",
        ...
        super().__init__()
        self.name = name
        self._sin_error_factor = sin_error_factor  #1.0
        self._num_class = num_class     #10
        self._use_rotate_nms = use_rotate_nms  #false
        self._multiclass_nms = multiclass_nms   #false
        self._nms_score_thresholds = nms_score_thresholds     #<class 'list'>: [0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806, 0.05000000074505806]
        self._nms_pre_max_sizes = nms_pre_max_sizes           #<class 'list'>: [1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000]
        self._nms_post_max_sizes = nms_post_max_sizes        #<class 'list'>: [300, 300, 300, 300, 300, 300, 300, 300, 300, 300]
        self._nms_iou_thresholds = nms_iou_thresholds           #<class 'list'>: [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]
        self._use_sigmoid_score = use_sigmoid_score            #true
        self._encode_background_as_zeros = encode_background_as_zeros        #true
        self._use_direction_classifier = use_direction_classifier           #true
        self._num_input_features = num_input_features                     #4
        self._box_coder = target_assigner.box_coder
        self.target_assigner = target_assigner
        self.voxel_generator = voxel_generator
        self._pos_cls_weight = pos_cls_weight           #1.0
        self._neg_cls_weight = neg_cls_weight            #1.0
        self._encode_rad_error_by_sin = encode_rad_error_by_sin         #true
        ...
        self._dir_offset = dir_offset             #0.779
        self._loc_loss_ftor = loc_loss_ftor
        self._cls_loss_ftor = cls_loss_ftor
        self._direction_loss_weight = direction_loss_weight          #0.2
        self._cls_loss_weight = cls_loss_weight                     #1.0
        self._loc_loss_weight = loc_loss_weight                     #2.0
        self._post_center_range = post_center_range or []
        self.measure_time = measure_time              # false
        self._nms_class_agnostic = nms_class_agnostic     # false
        self._num_direction_bins = num_direction_bins       # 2
        self._dir_limit_offset = direction_limit_offset      #0.0
        self.voxel_feature_extractor = voxel_encoder.get_vfe_class(vfe_class_name)(      #vfe_class_name=PillarFeatureNet
            num_input_features,
            use_norm,
        ...
        # self.voxel_radar_feature_extractor = voxel_encoder.get_vfe_class(vfe_radar_class_name)(  # vfe_class_name=PillarFeatureNet
        #     num_input_features,
        #     use_norm,
        #     num_filters=vfe_num_filters,
        #     with_distance=with_distance,
        ...
        #     pc_range=self.voxel_generator.point_cloud_range,
        # )
        self.middle_feature_extractor = middle.get_middle_class(middle_class_name)(     #middle_class_name=pointpillarscatter
            output_shape,
            use_norm,
        ...
        self.rpn = rpn.get_rpn_class(rpn_class_name)(     #RPNV2
            use_norm=True,
            num_class=num_class,     #10
            layer_nums=rpn_layer_nums,    #[3,5,5]
            layer_strides=rpn_layer_strides,    #[2,2,2]
            num_filters=rpn_num_filters,       #[64,128,256]
            upsample_strides=rpn_upsample_strides,   #[0.25,0.5,1]
            num_upsample_filters=rpn_num_upsample_filters,   #[128,128,128]
            num_input_features=rpn_num_input_features,         #[64]
            num_anchor_per_loc=target_assigner.num_anchors_per_location,   #20
            encode_background_as_zeros=encode_background_as_zeros,        #true
            use_direction_classifier=use_direction_classifier,            #true
            use_groupnorm=use_groupnorm,                                  #false
            num_groups=num_groups,                                        #32
            box_code_size=target_assigner.box_coder.code_size,            # 7
    ...
    def loss(self, example, preds_dict):
        box_preds = preds_dict["box_preds"]     #torch.Size([3, 20, 50, 50, 7])
        cls_preds = preds_dict["cls_preds"]     #torch.Size([3, 20, 50, 50, 10])
        a=cls_preds.shape[1]  #20
        b=box_preds.shape    #torch.Size([3, 20, 50, 50, 7])
        batch_size_dev = cls_preds.shape[0]               #3
        self.start_timer("loss forward")
        labels = example['labels']              #torch.Size([3, 50000])
        reg_targets = example['reg_targets']     #torch.Size([3, 50000, 7])
        importance = example['importance']        #torch.Size([3, 50000])
        ...
        loc_loss, cls_loss = create_loss(                #跳到747
            self._loc_loss_ftor,
            self._cls_loss_ftor,
            box_preds=box_preds,   #torch.Size([3, 20, 50, 50, 7])
            cls_preds=cls_preds,     #torch.Size([3, 20, 50, 50, 10])
            ...
            encode_background_as_zeros=self._encode_background_as_zeros,  #背景为0
    ...
    def network_forward(self, voxels, radar_voxels, num_points,radar_num_points, coors,radar_coors, batch_size):
        ...
        voxel_features = self.voxel_feature_extractor(voxels, num_points,coors,batch_size)      #(voxel_feature_extractor):PillarFeatureNet((pfn_layers): ModuleList(      #[41924,64]
        radar_voxel_feature = self.voxel_feature_extractor(radar_voxels, radar_num_points, radar_coors,batch_size)  #radar 特征提取方式                                                                # (0): PFNLayer((linear): DefaultArgLayer(in_features=9, out_features=64, bias=False) (norm): DefaultArgLayer(64, eps=0.001, momentum=0.01, affine=True, track_running_stats=True))))

        self.end_timer("voxel_feature_extractor")

        self.start_timer("middle forward")
        # spatial_features = self.middle_feature_extractor(      #PointPillarsScatter()扩展到画布
        #     voxel_features,radar_voxel_feature, coors,radar_coors, batch_size)
        spatial_features = self.middle_feature_extractor(  # PointPillarsScatter()扩展到画布
            voxel_features, coors, batch_size)
        radar_spatial_features = self.middle_feature_extractor(radar_voxel_feature,radar_coors,batch_size)
        a=spatial_features.shape   #[3,64,400,400]
        self.end_timer("middle forward")
        self.start_timer("rpn forward")
        features = torch.cat((spatial_features,radar_spatial_features),0)
        aaa=features.shape
        preds_dict = self.rpn(features)            #rpn:RPNV2
    ...
    def forward(self, example):
        ...
        """
        voxels = example["voxels"]         #shape 38093 60 4
        num_points = example["num_points"]            #size:38093
        coors = example["coordinates"]             #38093

        radar_voxels = example["radar_voxels"]        #radar's data
        radar_num_points = example["radar_num_points"]
        radar_coors = example["radar_coordinates"]
        ...
        batch_anchors = example["anchors"]            #(3,50000,7)
        batch_size_dev = batch_anchors.shape[0]       # 3

        # features: [num_voxels, max_num_points_per_voxel, 7]
        # num_points: [num_voxels]
        # coors: [num_voxels, 4]

        preds_dict = self.network_forward(voxels,radar_voxels, num_points,radar_num_points, coors, radar_coors, batch_size_dev)  # box_preds,cls_preds,dir_cls_preds
        # need to check size.
        box_preds = preds_dict["box_preds"].view(batch_size_dev, -1, self._box_coder.code_size)    #box预测 batch_size_dev:3    box_coder.code_size:7   box_preds=torch.Size([3, 50000, 7])
        err_msg = f"num_anchors={batch_anchors.shape[1]}, but num_output={box_preds.shape[1]}. please check size"     #[3,50000,7]
    ...
    def update_metrics(self, cls_loss, loc_loss, cls_preds, labels, sampled):#result of cls_loss, loc_loss, cls_preds, labels, sampled
        ...
        for i, thresh in enumerate(self.rpn_metrics.thresholds):
            ret["pr"][f"prec@{int(thresh*100)}"] = float(prec[i]) #准确率
            ret["pr"][f"rec@{int(thresh*100)}"] = float(recall[i])#召回率
        return ret
...
def add_sin_difference(boxes1, boxes2, boxes1_rot, boxes2_rot, factor=1.0):  #boxes2为label
    if factor != 1.0:
        boxes1_rot = factor * boxes1_rot    #torch.Size([3, 50000, 1])
        boxes2_rot = factor * boxes2_rot    #torch.Size([3, 50000, 1])
    rad_pred_encoding = torch.sin(boxes1_rot) * torch.cos(boxes2_rot)  #torch.Size([3, 50000, 1])
    rad_tg_encoding = torch.cos(boxes1_rot) * torch.sin(boxes2_rot)  #torch.Size([3, 50000, 1])
    boxes1 = torch.cat([boxes1[..., :6], rad_pred_encoding, boxes1[..., 7:]],  #torch.Size([3, 50000, 7])
                       dim=-1)
    boxes2 = torch.cat([boxes2[..., :6], rad_tg_encoding, boxes2[..., 7:]],  #box加上方向torch.Size([3, 50000, 7])
                       dim=-1)
...
def create_loss(loc_loss_ftor,
                ...
                box_code_size=7,
                num_direction_bins=2):
    batch_size = int(box_preds.shape[0])     #3
    box_preds = box_preds.view(batch_size, -1, box_code_size)
    if encode_background_as_zeros:
        cls_preds = cls_preds.view(batch_size, -1, num_class)  #num_class =10
    else:
        cls_preds = cls_preds.view(batch_size, -1, num_class + 1)
    cls_targets = cls_targets.squeeze(-1)
    one_hot_targets = torchplus.nn.one_hot(                 #torch.Size([3, 50000, 11])
        cls_targets, depth=num_class + 1, dtype=box_preds.dtype)
    if encode_background_as_zeros:
        one_hot_targets = one_hot_targets[..., 1:]         #torch.Size([3, 50000, 10])
    if encode_rad_error_by_sin:
        # sin(a - b) = sinacosb-cosasinb
        # reg_tg_rot = box_torch_ops.limit_period(
        #     reg_targets[..., 6:7], 0.5, 2 * np.pi / num_direction_bins)
        box_preds, reg_targets = add_sin_difference(box_preds, reg_targets, box_preds[..., 6:7], reg_targets[..., 6:7],  #box_preds:torch.Size([3, 50000, 7])    reg_targets:torch.Size([3, 50000, 7])
                                                    sin_error_factor)

    loc_losses = loc_loss_ftor(      #torch.Size([3, 50000, 7])
        box_preds, reg_targets, weights=reg_weights)  # [N, M]
    cls_losses = cls_loss_ftor(           #torch.Size([3, 50000, 10])
...
def prepare_loss_weights(labels,
    ...
    # cared: [N, num_anchors]
    positives = labels > 0
    negatives = labels == 0    #torch.Size([3, 50000])
    negative_cls_weights = negatives.type(dtype) * neg_cls_weight
    cls_weights = negative_cls_weights + pos_cls_weight * positives.type(dtype) #torch.Size([3, 50000])
    reg_weights = positives.type(dtype) #torch.Size([3, 50000])
...
def get_direction_target(anchors,
    ...
    anchors = anchors.view(batch_size, -1, anchors.shape[-1])       #torch.Size([3, 50000, 7])
    rot_gt = reg_targets[..., 6] + anchors[..., 6]                  #torch.Size([3, 50000])
    offset_rot = box_torch_ops.limit_period(rot_gt - dir_offset, 0, 2 * np.pi)  #torch.Size([3, 50000])
    dir_cls_targets = torch.floor(offset_rot / (2 * np.pi / num_bins)).long()      #torch.Size([3, 50000])
    dir_cls_targets = torch.clamp(dir_cls_targets, min=0, max=num_bins - 1)         #torch.Size([3, 50000])
    if one_hot:
